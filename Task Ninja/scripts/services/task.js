...
app.factory("Task", function(FURL, $firebase, Auth){
	...
	var Task = {
		...
		/* This is for checking does the certain user creator of the task or not */
		isCreator: function(task) {
			return ( user && user.provider && user.uid === task.poster );
		},
		/* Task status open or not */
		isOpen: function(task) {
			return task.status === "open";
		},
		//After pushing the completed button we just set task statue to completed
